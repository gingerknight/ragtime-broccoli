...
def print_rrf_reranked_cross_encoder(results: list, query: str, k, n: int = 5) -> None:
    print(f"Re-ranking top {n} results using cross_encoder method...")
    print(f"Reciprocal Rank Fusion Results for '{query}' (k={k})")
    for num, val in enumerate(results[:n]):
        print(f"{num + 1}. {val[1]['title']}")
        print(f"   Cross Encoder Score: {val[1]['cross_encoder_score']:.4f}")
        print(f"   RRF Score: {val[1]['rrf_score']:.4f}")
        print(f"   BM25 Rank: {val[1]['bm25_rank']}, Semantic Rank: {val[1]['semantic_rank']}")
        print(f"   {val[1]['document'][:100]}")


def rerank_gemini(query, results: list, q_method: str, k, limit) -> list:
