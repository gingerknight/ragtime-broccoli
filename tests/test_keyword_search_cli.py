...
def test_main_build_path_calls_build(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod.argparse.ArgumentParser, "parse_args", lambda _self: Namespace(command="build", query=None))
    monkeypatch.setattr(cli_mod.MovieSearch, "from_file", classmethod(lambda _cls: fake_ms))
...
def test_main_returns_2_when_loading_movies_fails(monkeypatch, capsys):
    monkeypatch.setattr(cli_mod.argparse.ArgumentParser, "parse_args", lambda _self: Namespace(command="search", query="x"))
    monkeypatch.setattr(cli_mod.MovieSearch, "from_file", classmethod(lambda _cls: (_ for _ in ()).throw(RuntimeError("boom"))))
...
def test_main_returns_2_when_cache_load_fails(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli_mod.argparse.ArgumentParser, "parse_args", lambda _self: Namespace(command="search", query="brave"))
    monkeypatch.setattr(cli_mod.MovieSearch, "from_file", classmethod(lambda _cls: fake_ms))
    monkeypatch.setattr(cli_mod.InvertedIndex, "load", staticmethod(lambda: (_ for _ in ()).throw(SearchEngineError("cache fail"))))
